...
console.log("Puppeteer version:", puppeteer?.product || "Unknown");
console.log("Puppeteer resolved path:", path.resolve("node_modules/puppeteer"));

const MAX_TABS = 3; // Maximum number of tabs allowed
...
async function getTab(browser) {
	...
	while (tabPool.length >= MAX_TABS) {
		await new Promise((resolve) => setTimeout(resolve, 100));
	}

	return getTab(browser);
}

async function releaseTab(tab) {
	const index = tabPool.indexOf(tab);
	if (index > -1) {
...
// Contoh penggunaan:
await randomDelay(2000, 5000); // Jeda antara 2-5 detik
...
rl.question("Masukkan keyword: ", (inputKeyword) => {
	rl.question("Masukkan jumlah halaman maksimum: ", (inputMaxPage) => {
		const keyword = inputKeyword;
		const maxPage = parseInt(inputMaxPage, 10);
			...
			for (let i = 0; i < productUrls.length; i++) {
				...
				try {
					...
					const data = await productPage.evaluate(() => {
						...
						const elements = document.querySelectorAll("div.some-class");
